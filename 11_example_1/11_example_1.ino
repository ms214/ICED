...
// adjust servo position according to the USS read value
  ...
  else if(dist_ema >= 360) myservo.writeMicroseconds(_DUTY_MAX);
  else myservo.writeMicroseconds(dist_min+10.26*((int)dist_ema-144));
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  //LED control
  if(reading >= 180 && reading <= 360) digitalWrite(PIN_LED, LOW);
